...
# Filters out failed breakouts (current close > breakout close)
import requests
import pandas as pd
import numpy as np
...
BODY_RATIO = 0.65  # Full-body threshold
EPS = 0.001  # Tolerance
...
def find_pumps(df):
    ...
    for n in N_LIST:
        ...
        eps_arr = prior_high * EPS
        cond = green_full & (c > (prior_high + eps_arr))
        for idx in np.where(cond)[0]:
            if idx >= len(df) - SEARCH_WINDOW:
                # Check if not failed: current close > breakout close
                if df["c"].iloc[-1] > df["c"].iloc[idx]:
                    pumps.append((idx, n))
    return pumps
