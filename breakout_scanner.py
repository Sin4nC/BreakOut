...
# 4) bottom wick <= 1 tick
# 5) untouched = only NEXT candle must NOT take signal H or L
...
UNTOUCHED_MODE = "next"   # "next" or "all"
SEARCH_WINDOW = 800
LIMIT = 500
MAX_WORKERS = 12
FALLBACK_TICK = 1e-6
...
def fetch_klines(symbol, interval=INTERVAL, limit=LIMIT):
    ...
    if r.status_code != 200:
        p["interval"] = "Hour4"  # fallback name seen on some gateways
        r = SES.get(url, params=p, timeout=25)
    r.raise_for_status()
    rows = r.json()
...
def find_hits(df, tick, sym=None):
    ...
    # quantize OHLC on tick grid
    ot = ticks_round(o, tick)
    ct = ticks_round(c, tick)
    ht = ticks_ceil(h, tick)    # be conservative for highs
    lt = ticks_floor(l, tick)   # be conservative for lows

    highs = pd.Series(ht, copy=False)
    prior_highs_ticks = {
        nlook: highs.rolling(nlook).max().shift(1).astype("Int64").fillna(pd.NA)
        for nlook in N_LIST
    }

    # future extremes for untouched all
    ...
    for i in range(start_i, n):
        ...
        for nlook in N_LIST:
            ph = prior_highs_ticks[nlook].iat[i]
            if pd.notna(ph) and ct[i] > int(ph):
...
def run_all(symbols, tick_map):
    ...
    rows.sort(key=lambda r: (r["candles_ago"], r["symbol"]))
    return rows
