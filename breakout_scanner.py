...
def get_symbols():
    r = requests.get(f"{BASE}/api/v3/exchangeInfo", timeout=25)  # Fixed to exchangeInfo for accurate spot trading status
    r.raise_for_status()
    syms = []
...
def fetch_klines(symbol, interval=INTERVAL, limit=LIMIT):
    ...
    if not rows or len(rows[0]) < 5:
        return None
    cols = ["t","o","h","l","c","v","ct","qv","n","tb","tqv","i"]  # 12 columns
